...
function renderUniversalGame(id) {
  ...
  function card(r) {
    ...
    if (r.type === 'quest') {
      ...
      body = `<p>${p.claimed || 0} / ${ruleClaimLimit(g, r)}${r.monthlyLimit ? ` · 이번 달 ${p.monthCount || 0}/${r.monthlyLimit}` : ''}</p><div class="catalog-actions">${r.rewards.map(x => button(r, 'complete', x.label, `data-reward="${x.id}"`, blocked || catalogDone(g, r))).join('')}</div>`;
    } else if (r.type === 'resource') {
      const clock = catalogResourceClock(r, p);
      body = `<strong class="catalog-value">${p.value} / ${r.capacity}</strong><p data-clock="${r.id}">${clock.label}</p><div class="win-progress"><span data-clock-bar="${r.id}" style="width:${clock.percent}%"></span></div>${button(r, 'consume', r.consumeLabel || '1개 사용', '', blocked || p.value < 1)}`;
    } else {
      ...
      body = `<strong class="catalog-value">${value} / ${r.target}</strong><div class="win-progress"><span style="width:${Math.min(100, (value / r.target) * 100)}%"></span></div>${r.totalResource ? `<p>누적 ${(totals(id)[r.totalResource] || 0).toLocaleString()} ${escapeHtml(REWARD_NAMES[r.totalResource] || r.totalResource)}</p>` : ''}${!r.hideAction ? `<div class="catalog-actions">${button(r, 'progress', r.eventLabel || '+1', 'data-amount="1"', blocked || (!r.event && value >= r.target))}<input data-progress-value="${r.id}" type="number" min="${value}" max="${r.target}" value="${value}" aria-label="${escapeHtml(r.label)} 진행도" />${button(r, 'set-progress', '진행도 반영', '', blocked)}</div>` : ''}`;
      if (!r.window && steps.length > 12)
        body += `<div class="catalog-pages"><button data-step-page="${r.id}" data-delta="-1" ${page === 0 ? 'disabled' : ''}>‹</button><span>${page + 1} / ${Math.ceil(steps.length / 12)}</span><button data-step-page="${r.id}" data-delta="1" ${(page + 1) * 12 >= steps.length ? 'disabled' : ''}>›</button></div>`;
      if (shown.length)
    ...
    return `<section class="compact-card catalog-card" data-card="${r.id}" style="grid-column:span ${Math.min(3, Math.max(1, r.span || 1))}"><h3>${escapeHtml(r.label)}</h3><small>${escapeHtml(ruleScheduleText(r))}${blocked ? ' · 진행 불가' : ''}</small>${r.note ? `<p>${escapeHtml(r.note)}</p>` : ''}${body}</section>`;
  }
  ...
  $('#questList').innerHTML = `<div class="catalog-grid">${rules
    .filter(r => !r.page)
    .map(card)
    .join('')}</div>${
    groups.length
      ? `<div class="catalog-pages"><button data-catalog-page="-1" ${groups.indexOf(g.catalogPage) <= 0 ? 'disabled' : ''}>‹</button><strong>${escapeHtml(g.catalogPage)}</strong><button data-catalog-page="1" ${groups.indexOf(g.catalogPage) >= groups.length - 1 ? 'disabled' : ''}>›</button></div><div class="catalog-grid">${rules
          .filter(r => r.page === g.catalogPage)
          .map(card)
          .join('')}</div>`
      : ''
