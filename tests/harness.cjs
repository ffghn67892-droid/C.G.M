...
function parse(markup) {
  return new DOMParser({
    ...
    markup.replace(/\b(hidden|disabled|selected|checked)(?=[\s>])/g, '$1="$1"'),
    'text/html'
  );
}
...
function matches(node, selector) {
  ...
  for (const [, name, value] of selector.matchAll(/\[([\w-]+)(?:="([^"]*)")?\]/g)) {
    if (!node.hasAttribute(name) || (value !== undefined && node.getAttribute(name) !== value))
      return false;
  }
  ...
  if ([...selector.matchAll(/\.([\w-]+)/g)].some(match => !classes.includes(match[1])))
    return false;
...
elementProto.closest = function (selector) {
  let node = this;
  while (node && !matches(node, selector)) node = node.parentNode;
  return node;
};
...
elementProto.click = function () {
  if (!this.hasAttribute('disabled'))
    for (const callback of listeners.get(this)?.click || []) callback({ target: this });
};
Object.defineProperties(elementProto, {
  ...
  checked: {
    ...
    set(v) {
      if (v) this.setAttribute('checked', 'checked');
  ...
  value: {
    get() {
      if (this.tagName === 'select') {
        ...
        return (
          (options.find(o => o.hasAttribute('selected')) || options[0])?.getAttribute('value') || ''
        );
      }
    ...
    set(v) {
      if (this.tagName === 'select')
        Array.from(this.getElementsByTagName('option')).forEach(o => {
          if (o.getAttribute('value') === String(v)) o.setAttribute('selected', 'selected');
  ...
  dataset: {
    get() {
      return new Proxy(
        ...
        {
          get: (_, key) =>
            this.getAttribute('data-' + key.replace(/[A-Z]/g, letter => '-' + letter.toLowerCase()))
        }
      );
    }
  },
...
function start(saved = null) {
  ...
  let stored = saved;
  const storage = new Map(saved ? [['deckroom-quests', saved]] : []);
  ...
  const context = vm.createContext({
    ...
    localStorage: {
      ...
      setItem: (key, value) => {
        ...
        if (key === 'deckroom-quests') stored = value;
      },
  ...
  if (!saved)
    vm.runInContext(
      "for(const [id]of GAMES){state.games[id].profile.registeredAt=new Date().toISOString();}state.activeGame='kards';renderAll();",
      context
    );
  return {
    document,
    ...
    rows() {
      return document.querySelectorAll('.queued-row');
    }
  };
}
