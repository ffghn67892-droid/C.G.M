...
test('gauge clamps to max, tracks milestones as sticky, and disappears only at true max (not personal target)', () => {
  const a = start();
  seed(a, [
    { id: 'win', name: '승리', format: 'daily', kind: 'gauge', min: 0, max: 3, milestones: [2] }
  ]);
  for (let i = 0; i < 5; i++) a.run("catalogAction('kards','win','increment')");
  assert.equal(
    a.run('ruleProgress(state.games.kards, catalogRules(state.games.kards)[0]).value'),
    3,
    'clamped to max'
  );
  assert.deepEqual(
    a.run('ruleProgress(state.games.kards, catalogRules(state.games.kards)[0]).achievedMilestones'),
    [2]
  );
  assert.equal(a.run("catalogAction('kards','win','set-fold-target',{value:1})"), true);
  assert.equal(
    a.run('universalStatus(state.games.kards).count'),
    0,
    'true max reached: not counted as remaining even though foldTarget is lower'
  );
});
...
test('undo reverses exactly the last action, in order, and is a no-op when history is empty', () => {
  ...
  assert.equal(
    a.run('ruleProgress(state.games.kards, catalogRules(state.games.kards)[1]).value'),
    1
  );
  assert.equal(a.run("catalogAction('kards','g','undo')"), true);
  assert.equal(
...
test('universalStatus counts only true remaining work and flags urgent when a slot is at capacity', () => {
  ...
  a.run(
    "catalogAction('kards','s1','complete');catalogAction('kards','s1','complete');catalogAction('kards','s1','complete')"
  );
