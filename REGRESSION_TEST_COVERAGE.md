...
- **`tests/catalog-critical.test.cjs` 전체 삭제(10개)**: E1~E8 전부 quest/claim/goal/resource/pass 타입의 보상 지급·연동·중복 방지를 검증하던 것으로, 대상 기능 자체가 설계상 사라졌다. 재작성 대상이 아니라 폐기 대상이다.
- **`tests/catalog-additional.test.cjs`에서 4개 삭제**: "muted Duel Links..."(리셋 알림이 등록된 규칙 유무와 무관하게 존재한다고 가정했으나, 새 모델에서는 규칙이 없는 게임은 리셋 알림 자체가 없는 게 올바른 동작), "catalog schedules use KST weekly..."(옛 `schedule.kind`/`intervalDays` 스키마 직접 참조), "universalStatus counts..."·"period reset clears...collapsed..."(Stage K2의 `p.collapsed` 필드 자체가 최종 설계에서 빠짐 — 동일 요구사항은 `tests/catalog-engine-v2.test.cjs`가 새 스키마 기준으로 이미 다룬다).
