      ...
      registered.length
        ? `<div class="overview-grid">${registered.map(
            ([id, name]) => {
              ...
              return `<article class="overview-card" data-game-name="${escapeHtml(name.toLowerCase())}"><h3><span class="status-dot ${s.color} ${s.color === 'done' ? '' : 'status-dot-count'}">${statusBadgeText(s)}</span>${escapeHtml(name)}</h3><p class="overview-totals">${escapeHtml(rewardText(totals(id)))}</p><dl><dt>현금 지출</dt><dd>${spendingText(p)}</dd><dt>이용 일수</dt><dd>경과 ${days}일 · 활동 ${p.activityDays.length}일</dd><dt>패스</dt><dd>${p.pass.active ? '활성' : '미활성'} · Lv. ${p.pass.level}</dd><dt>종료</dt><dd>${p.pass.end ? escapeHtml(new Date(p.pass.end).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' })) : '미지정'}</dd>${id === 'master-duel' && !state.games[id].masterDuel?.eventDeleted && Date.now() < Date.parse(DICE_END) ? '<dt>이벤트 종료</dt><dd>9월 21일 12:59 KST</dd>' : ''}</dl><div class="card-actions"><button data-open-game="${id}">게임 열기</button><button data-settings="${id}">상세 설정</button><button data-mute="${id}">${p.mutedUntil > Date.now() ? '오늘 알람 꺼짐' : '오늘의 알람 끄기'}</button></div></article>`;
            }
          ).join('')}</div>`
