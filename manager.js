...
function renderCurrentGame(updateNavigation = true) {
  renderUniversalGame(state.activeGame);
  if (updateNavigation) renderNavigation();
}
function renderManaged() {
  ...
  if (!toolbar) {
    ...
    $('.page-heading').appendChild(toolbar);
  }
  toolbar.innerHTML = '';
  if (state.activeGame === 'overview') {
    renderOverview();
    save();
    return true;
  }
  const id = state.activeGame,
    p = data().profile;
  if (!p.registeredAt) {
    $('#questList').innerHTML =
  ...
  $('#gameSettings').addEventListener('click', () => openSettings(id));
  $('#gameMute').addEventListener('click', () => muteGame(id));
  $('#gameAlarmStop').addEventListener('click', () => {
    acknowledge(id);
    save();
    toast('알람을 중지했습니다.');
  });
...
function attachRendererShell() {
  ...
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden) refreshAllGames(true);
  });
  window.deckroom?.onResume(() => refreshAllGames(true));
  window.deckroom?.setTray(!!state.tray);
  document.addEventListener('keydown', e => {
    const modal = $('#managerDialog');
    if (!modal || modal.hidden) return;
    if (e.key === 'Escape') closeDialog();
    if (e.key === 'Tab') {
      const elements = [...modal.querySelectorAll('*')].filter(
        x =>
          ['BUTTON', 'INPUT', 'SELECT', 'TEXTAREA'].includes(x.tagName.toUpperCase()) &&
...
const RENDERER_DEPENDENCIES = Object.freeze([
  'migrateStore',
  'freshGame',
  'renderManaged',
  'attachRendererShell',
