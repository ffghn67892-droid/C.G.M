...
let stored=null;
try { stored=JSON.parse(localStorage.getItem('deckroom-quests')||'null'); } catch { localStorage.setItem('deckroom-corrupt-backup',localStorage.getItem('deckroom-quests')||''); }
const state = stored?.games ? stored : { activeGame: 'kards', games: Object.fromEntries(GAMES.map(([id]) => [id, { generalDate: '', generalMissions: [], completedMissions: [], refreshed: [], seasonQuests: [], weeklyCount: 0, rewards: { credits: 0, seasonXp: 0 } }])) };
for(const entry of state.customGames||[])if(!GAMES.some(([id])=>id===entry[0]))GAMES.push(entry);
...
function data() { return state.games[state.activeGame]; }
...
function setQuestSummary(label, value) { $('#questSummary').hidden = false; $('#questSummaryLabel').textContent = label; $('#questSummaryValue').textContent = value; }
function clearQuestSummary() { $('#questSummary').hidden = true; }
