...
function ensureMtgaData(current, now = new Date()) {
  ...
  if (!current.mtga.dailyQueueExpanded) {
    ...
    while (queue.missions.length < 3)
      queue.missions.push({ id: `mtga-${crypto.randomUUID()}`, label: '일일 퀘스트' });
    current.mtga.dailyQueueExpanded = true;
  }
  const queue = syncDailyQueue(current.mtga, 'dailyQueue', 1, false, now);
  queue.missions.forEach(mission => {
    if (mission.label === '일일 퀘스트가 갱신되었습니다.') mission.label = '일일 퀘스트';
  });
  if (!queue.unifiedDailyView) {
    if (current.mtga.dailyQuest === false && queue.missions[0]) {
      const reward = [500, 750].includes(current.mtga.questReward)
        ? current.mtga.questReward
        : undefined;
...
function ensureMasterDuelData(current) {
  ...
  if (!Number.isFinite(current.masterDuel.rewardTotals.gems))
    current.masterDuel.rewardTotals.gems = 0;
...
function ensureDuelLinksData(current) {
  if (!current.duelLinks)
    current.duelLinks = {
      normalDuelists: 10,
      replenishmentStartedAt: null,
      weeklyDone: [],
      rewardTotals: { gems: 0, rJewels: 0, blueGateKeys: 0, gold: 0 }
    };
  ...
  for (const key of ['gems', 'rJewels', 'blueGateKeys', 'gold'])
    if (!Number.isFinite(current.duelLinks.rewardTotals[key]))
      current.duelLinks.rewardTotals[key] = 0;
  return current.duelLinks;
}

function ensurePokemonPocketData(current, now = new Date()) {
  if (!current.pokemonPocket)
    current.pokemonPocket = {
      freePacks: 2,
      getChallengePoints: 5,
      lastPackRecoveryAt: now.getTime(),
      lastChallengeRecoveryAt: now.getTime(),
      dailyDone: Array.isArray(current.pokemonDone) ? current.pokemonDone : [],
      dailyRewardClaimed: false,
      rewardTotals: { challengeHourglasses: 0, packHourglasses: 0, freePackOpens: 0 }
    };
  ...
  for (const key of ['challengeHourglasses', 'packHourglasses', 'freePackOpens'])
    if (!Number.isFinite(pocket.rewardTotals[key])) pocket.rewardTotals[key] = 0;
  ...
  if (!Number.isFinite(pocket.lastChallengeRecoveryAt))
    pocket.lastChallengeRecoveryAt = now.getTime();
  return pocket;
}
...
function syncPokemonPocket(current, now = new Date()) {
  ...
  const recover = (countKey, timestampKey, maximum) => {
    if (pocket[countKey] >= maximum) {
      pocket[countKey] = maximum;
      return;
    }
  ...
  else if (period > pocket.dailyPeriod) {
    pocket.dailyDone = [];
    pocket.dailyRewardClaimed = false;
    pocket.dailyPeriod = period;
  }
  if (!pocket.dailyRewardClaimed && pocket.dailyDone.length >= 3) {
    ...
    award(
      'pokemon-pocket',
      `daily/${period}`,
      { challengeHourglasses: 2, packHourglasses: 3 },
      '일일 미션'
    );
  }
  return pocket;
...
function syncDuelLinks(current, now = new Date()) {
  ...
  else if (period > duelLinks.weeklyPeriod) {
    duelLinks.weeklyDone = [];
    duelLinks.weeklyPeriod = period;
  }
  if (duelLinks.replenishmentStartedAt !== null) {
    ...
    duelLinks.normalDuelists = Math.min(10, duelLinks.normalDuelists + recovered);
    duelLinks.replenishmentStartedAt += recovered * 30 * 60 * 1000;
...
function normalizeMasterDuelQuestLabels(masterDuel, queue) {
  const used = new Set();
  queue.missions.forEach(mission => {
    if (MASTER_DUEL_QUESTS.includes(mission.label) && !used.has(mission.label))
      used.add(mission.label);
    else mission.label = '';
  });
  queue.missions.forEach(mission => {
    if (!mission.label)
      mission.label = nextMasterDuelQuest(masterDuel, used) || MASTER_DUEL_QUESTS[0];
  });
  return used;
}

function addMasterDuelQuests(
  masterDuel,
  queue,
  count,
  used = normalizeMasterDuelQuestLabels(masterDuel, queue)
) {
...
function syncKardsMissions(now = new Date()) {
  return syncKardsRules(data(), now);
}
...
function syncRetainedDaily(
  queue,
  { dateField = 'date', count = 1, label = '일일 퀘스트', gameId = state.activeGame } = {},
  now = new Date()
) {
  ...
  if (queue.resetSchedule !== 'kst-v1') {
    ...
    for (let index = 0; index < count && queue.missions.length < 3; index += 1)
      queue.missions.push({ id: crypto.randomUUID(), label, done: false });
    return queue;
  }
...
function ensureShadowverseData(current) {
  ...
  for (const key of ['rupies', 'redEther', 'keys']) {
    if (!Number.isFinite(current.shadowverse.rewardTotals[key]))
      current.shadowverse.rewardTotals[key] = 0;
  }
...
function syncGameProgress(id = state.activeGame, now = new Date()) {
  ...
  if (id === MTGA_ID) {
    ...
    advancePeriod(mtga, 'dailyWinPeriod', dailyPeriod(id, now), () => {
      mtga.dailyWins = 0;
    });
    advancePeriod(mtga, 'weeklyWinPeriod', weeklyPeriod(id, now), () => {
      mtga.weeklyWins = 0;
    });
  } else if (id === SNAP_ID) {
    ensureSnapCreditData(current);
    advancePeriod(current, 'weeklyResetPeriod', weeklyPeriod(id, now), () => {
      current.weeklyCount = 0;
      current.weeklyCreditClaims = [];
    });
  } else if (id === MIGHT_ID) {
    const might = ensureMightData(current, now);
    advancePeriod(might, 'weeklyResetPeriod', weeklyPeriod(id, now), () => {
      might.weeklyRemaining = true;
    });
...
function ensureParkData(shadowverse, now = new Date()) {
  ...
  advancePeriod(park, 'dailyPeriod', dailyPeriod('shadowverse', now), () => {
    park.dailyDone = [];
  });
  advancePeriod(park, 'weeklyPeriod', weeklyPeriod('shadowverse', now), () => {
    park.weeklyDone = [];
    park.points = 0;
    park.claimedMilestones = [];
  });
  return park;
}

function syncSnapMissions(now = new Date()) {
  if (state.activeGame !== SNAP_ID) return;
  const g = data(),
    slot = snapSlot(now);
  g.generalMissions ||= [];
  g.completedMissions ||= [];
  if (g.snapSlot === undefined) {
    g.snapSlot = slot;
    if (!g.generalMissions.length && !g.generalDate) g.snapSlot = slot - 1;
    g.generalMissions.forEach((m, i) => {
      m.difficulty ||= i % 2 ? 'hard' : 'normal';
    });
  }
  if (slot > g.snapSlot) {
    const count = Math.min(3, slot - g.snapSlot);
    for (let n = count - 1; n >= 0; n--)
      for (const difficulty of ['normal', 'hard'])
        if (g.generalMissions.length < 6)
          g.generalMissions.push({
            id: `slot/${slot - n}/${difficulty}`,
            difficulty,
            label: difficulty === 'normal' ? '일반 임무' : '어려움 임무'
          });
    g.snapSlot = slot;
    g.generalDate = String(dailyPeriod(SNAP_ID, now));
  }
}
...
function syncLegacyGame(id, now = new Date()) {
  ...
  const previous = state.activeGame;
  state.activeGame = id;
  try {
    if (g.profile.pass.end && Date.parse(g.profile.pass.end) <= now.getTime()) {
      g.profile.pass.active = false;
      if (id === 'kards' || id === 'might-magic') rewardOwner(id).passActive = false;
    }
    syncGameProgress(id, now);
    if (id === 'snap') syncSnapMissions(now);
    else if (id === 'kards') syncKardsMissions(now);
    else if (id === 'mtga') ensureMtgaData(g, now);
    else if (id === 'might-magic') ensureMightData(g, now);
    else if (id === 'master-duel') syncMasterDuelQuests(g, now);
    else if (id === 'duel-links') syncDuelLinks(g, now);
    else if (id === 'pokemon-pocket') syncPokemonPocket(g, now);
    else syncDailyQueue(g, 'dailyQueue', id === 'shadowverse' ? 3 : 1, id === 'shadowverse', now);
    const o = rewardOwner(id);
    if (id === 'might-magic' || id === 'hearthstone') {
      o.weeklyDone ||= [];
      advancePeriod(o, 'extraWeek', weeklyPeriod(id, now), () => (o.weeklyDone = []));
    }
    if (id === 'might-magic') {
      advancePeriod(o, 'heroPeriod', dailyPeriod(id, now), () => (o.heroesDailyRemaining = true));
      advancePeriod(o, 'loginPeriod', periodAt(4, 40, now), () => {
        o.loginClaimed = false;
        o.loginDay = (o.loginDay % 7) + 1;
      });
    }
    if (id === 'master-duel') {
      const month = new Date(now.getTime() + 9 * HOUR_MS).toISOString().slice(0, 7);
      if (o.loginMonth !== month) {
        o.loginMonth = month;
        o.loginCount = 0;
      }
      o.loginCount ||= 0;
    ...
    if (g.profile.pass.end && Date.parse(g.profile.pass.end) <= now.getTime()) {
      g.profile.pass.active = false;
      if (id === 'kards' || id === 'might-magic') o.passActive = false;
    }
  } finally {
